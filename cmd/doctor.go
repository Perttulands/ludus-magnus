...
func newDoctorCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&providerName, "provider", "anthropic", "Provider name (anthropic, openai-compatible, claude-cli, or pi-cli)")
...
func normalizeDoctorProvider(raw string) string {
	...
	if normalized == "pi" || normalized == "pi_cli" || normalized == "pi-cli" || normalized == "pi-ollama" || normalized == "ollama" {
		return "pi-cli"
	}
	return normalized
}
